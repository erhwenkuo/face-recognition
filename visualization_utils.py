...
def draw_bounding_box_on_image_array(image,
  ...
  """將邊界框添加到圖像 (numpy array).

  Args:
    image: 一個有著[height, width, 3]形狀的numpy array。
    ymin: 邊界框的ymin。
    xmin: 邊界框的xmin。
    ymax: 邊界框的ymax。
    xmax: 邊界框的xmax。
    color: 繪製邊界框的顏色。 預設值是'red'。
    thickness: 線的粗細。預設值是4。
    display_str_list: 要在邊界框中顯示的字符串列表
                      (每個都在自己的行中顯示)。
    use_normalized_coordinates: 如果為True（預設），則ymin，xmin，ymax，xmax為相對座標。否則為絕對座標。
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
...
def draw_bounding_boxes_on_image(image,
  ...
  Args:
    image: 一個PIL.Image對象。
    boxes: 一個2維的numpy array of [N, 4]: (ymin, xmin, ymax, xmax)。
           坐標採用[0,1]之間的標準化格式。
    color: 繪製邊界框的顏色。 預設值是'red'。
    thickness: line thickness. Default value is 4.
    display_str_list_list: list of list of strings.
